...
void LinkerSmallObjectAllocator::alloc_page() {
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LINZJ", "alloc_page: map_ptr: %p.\n", map_ptr);
...
LinkerMemoryAllocator::LinkerMemoryAllocator() {
  static const char* allocator_names[kSmallObjectAllocatorsCount] = {
    "linzj_alloc_16", // 2^4
    "linzj_alloc_32", // 2^5
    "linzj_alloc_64", // and so on...
    "linzj_alloc_128",
    "linzj_alloc_256",
    "linzj_alloc_512",
    "linzj_alloc_1024", // 2^10
  };
...
void* LinkerMemoryAllocator::alloc_mmap(size_t size) {
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "LINZJ", "alloc_mmap: map_ptr: %p.\n", map_ptr);

  if (map_ptr == MAP_FAILED) {
    __libc_fatal("mmap failed");
  }

  prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, map_ptr, allocated_size, "linzj_alloc_lob");
...
page_info* LinkerMemoryAllocator::get_page_info(void* ptr) {
  ...
  if (memcmp(info->signature, kSignature, sizeof(kSignature)) != 0) {
    __libc_fatal("invalid pointer %p (page signature mismatch, %x %x %x %x)", ptr, info->signature[0], info->signature[1], info->signature[2], info->signature[3]);
  }
